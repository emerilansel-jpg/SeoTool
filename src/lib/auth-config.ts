    ...
    plugins: [
      ...
      genericOAuth({
        config: [
          ...
          {
            // Separate provider so the GA4 grant carries only its own scope
            // (analytics.readonly) — least-privilege per connection, and the GSC
            // grant never needs re-consent when a user adds Analytics.
