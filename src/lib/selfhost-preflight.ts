...
function checkAuthMode(env: EnvRecord, items: PreflightItem[]): void {
  ...
  // The app is hosted-only now; AUTH_MODE is always "hosted" (or unset, which
  // defaults to hosted). Warn if a deprecated mode is set so operators notice.
  if (rawMode && !(AUTH_MODES as readonly string[]).includes(rawMode)) {
    items.push({
      key: "auth",
      name: "AUTH_MODE",
      level: "warn",
      message: `"${rawMode}" is deprecated and ignored. The app runs in hosted mode (Better Auth) only.`,
    });
  }
