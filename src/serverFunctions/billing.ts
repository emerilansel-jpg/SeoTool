...
export const getCustomerPortalUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    // Only paid customers can access the Stripe portal — free tier has
    ...
    const publicUrl = await getRequiredEnvValue("BETTER_AUTH_URL");
    const returnUrl = `${publicUrl}/billing`;
