...
/**
 * Kick off the incremental Google Analytics 4 OAuth grant. On success this
 * redirects the whole page to Google's consent screen; `callbackURL` is where
 * Google returns the user afterward. Shared by the connect card and the project
 * settings section so the link/error/redirect flow stays in one place — callers
 * keep their own analytics/dismissal at the call site.
 */
