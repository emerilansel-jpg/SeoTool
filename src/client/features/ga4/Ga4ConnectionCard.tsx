...
import { Ga4SelfHostedSetupWarning } from "@/client/features/ga4/SelfHostedSetupWarning";
import {
  PropertyPicker,
...
export function Ga4ConnectionCard({ projectId }: { projectId: string }) {
  ...
  const setPropertyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: GRANT_STATUS_KEY });
      // The GA4 report caches {connected:false}; refresh it so the page shows
      // data right after connecting instead of the stale card.
      void queryClient.invalidateQueries({
        queryKey: ["ga4Report", projectId],
      });
      void queryClient.invalidateQueries({
        queryKey: ["ga4ReportTable", projectId],
      });
      // The dashboard embeds this card and swaps it for the GA4 stats card once
      // activation reports the connection.
      void queryClient.invalidateQueries({
  ...
  const disconnectMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // shared grant-status cache honest.
      void queryClient.invalidateQueries({ queryKey: GRANT_STATUS_KEY });
      void queryClient.invalidateQueries({
        queryKey: ["ga4Report", projectId],
      });
      ...
      {connectionQuery.isLoading ? (
        ...
        <div className="space-y-4">
          <p className="text-sm text-base-content/70">
            Connect Google Analytics 4 to see traffic, sessions, users, and
            engagement for this website.
          </p>
          <button
            type="button"
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 rounded-lg border border-base-300 bg-base-200/40 p-3.5">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate font-mono text-xs text-base-content/55">
            {propertyId}
          </p>
          {connectedByEmail ? (
