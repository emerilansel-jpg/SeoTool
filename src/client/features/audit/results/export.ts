...
import type { getContentScores } from "@/serverFunctions/content-intelligence";

type ContentScoreRow = Awaited<
  ReturnType<typeof getContentScores>
>["scores"][number];
...
function contentRows(scores: ContentScoreRow[]): CsvValue[][] {
  return scores.map((s) => [
    s.url,
    s.score,
    s.depthScore,
...
export function exportContentScores(
  scores: ContentScoreRow[],
  ...
  if (format === "json") {
    const rows = scores.map((s) => ({
      ...
      headingsScore: s.headingsScore,
      metadataScore: s.metadataScore,
      mediaScore: s.mediaScore,
      linkingScore: s.linkingScore,
      technicalScore: s.technicalScore,
      wordCount: s.wordCount,
      issues: s.flags.length,
      flags: s.flags,
    }));
    downloadFile(
  ...
  const rows = contentRows(scores);

  if (format === "sheets") {
    void exportTableToSheets({
      headers: CONTENT_HEADERS,
      rows,
      feature: "content_scores",
    });
    return;
  }

  downloadCsv("content-scores.csv", buildCsv(CONTENT_HEADERS, rows));
}
