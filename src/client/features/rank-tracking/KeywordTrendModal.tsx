...
import { SerpSnapshotViewer } from "@/client/features/serp-snapshots/SerpSnapshotViewer";
...
export function KeywordTrendModal({
  ...
  locationName?: string;
  serpDepth: number;
  configDevices?: "both" | "desktop" | "mobile";
  onClose: () => void;
}) {
  const [sinceDays, setSinceDays] = useState(730);
  const [activeTab, setActiveTab] = useState<"history" | "serp">("history");

  const defaultDevice: "desktop" | "mobile" =
    configDevices === "desktop"
      ? "desktop"
      : configDevices === "mobile"
        ? "mobile"
        : "desktop";
  ...
  return (
    ...
    >
      ...
      {/* Tab toggle */}
      <div role="tablist" className="tabs tabs-boxed tabs-sm mt-3 mb-2">
        <button
          role="tab"
        ...
        <button
          role="tab"
          className={`tab ${activeTab === "serp" ? "tab-active" : ""}`}
