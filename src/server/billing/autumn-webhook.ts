...
export async function handleAutumnWebhookRequest(request: Request) {
  ...
  //
  // We handle both the legacy billing.updated event and the subscription.*
  // lifecycle events. All paths converge on syncAntumnCustomerStatus, which
  // re-reads the full customer, derives the plan tier, and resets quotas if
  // the tier changed.
  ...
  if (SYNC_EVENTS.has(payload.type)) {
    ...
    try {
      ...
      console.error(`Autumn ${payload.type} sync failed`, customerId, error, {
