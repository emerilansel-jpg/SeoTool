...
export async function getOrCreateOrganizationCustomer(
  ...
  // read, even before their first Autumn webhook fires.
  try {
 ...
 *  Reads from the local subscription table (synced from Autumn webhooks) so
 *  it's fast and doesn't add an Autumn round-trip to the hot path. */
