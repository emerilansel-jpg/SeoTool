...
import {
  ...
  type SerpSnapshotInsertRow,
} from "@/server/features/serp-snapshots/SerpSnapshotRepository";
import {
...
/** Expand RankCheckResult.serpItems into insert rows for serp_snapshots. */
function mapResultsToSerpSnapshotRows(
  runId: string,
  results: RankCheckResultWithDevice[],
): SerpSnapshotInsertRow[] {
  const rows: SerpSnapshotInsertRow[] = [];
  for (const r of results) {
    if (!r.serpItems) continue;
    for (const item of r.serpItems) {
      rows.push({
        ...
        domain: item.domain,
        isTrackedDomain: item.isTrackedDomain,
      });
    }
  }
  return rows;
...
async function checkBatchLive(
  ...
  if (results.length > 0) {
    ...
    // Persist full SERP composition (competitor snapshot) — zero extra API cost.
    const serpRows = mapResultsToSerpSnapshotRows(ctx.runId, results);
    if (serpRows.length > 0) {
      try {
        await SerpSnapshotRepository.insertBatch(serpRows);
      } catch (error) {
        console.warn(
          `[rank-check] ${ctx.runId} SERP snapshot insert failed:`,
          error,
        );
      }
    }
...
async function collectQueuedRound(
  ...
  if (completed.length > 0) {
    ...
    // Persist full SERP composition (competitor snapshot) — zero extra API cost.
    const serpRows = mapResultsToSerpSnapshotRows(ctx.runId, completed);
    if (serpRows.length > 0) {
      try {
        await SerpSnapshotRepository.insertBatch(serpRows);
      } catch (error) {
