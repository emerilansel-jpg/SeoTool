...
export async function runAuditPhases(
  ...
  await runContentScoringPhase(step, { auditId, workflowInstanceId });
  await runEntityExtractionPhase(step, { auditId, workflowInstanceId });
...
// Content-quality scoring runs after the crawl, once pages are persisted.
// It is purely computational (no external API) and best-effort: a failure
// here must not fail the whole audit, so the phase swallows its own errors.
async function runContentScoringPhase(
  step: WorkflowStep,
  params: { auditId: string; workflowInstanceId: string },
) {
  await pgStep(step, "content-scoring", undefined, async () => {
    try {
      const { scored } = await ContentIntelligenceService.computeScoresForAudit(
        params.auditId,
      );
      return { scored };
    } catch (error) {
      console.warn(
        `Content scoring failed for audit ${params.auditId}:`,
        error,
      );
      return { scored: 0 };
    }
...
// Per-page entity/topic extraction via LLM (OpenRouter). Runs after content
// scoring, once body_text is persisted. Best-effort: if OpenRouter is not
// configured or the phase fails, the audit continues without entity data.
async function runEntityExtractionPhase(
  step: WorkflowStep,
  params: { auditId: string; workflowInstanceId: string },
) {
  await pgStep(step, "entity-extraction", undefined, async () => {
    try {
      const { extracted } =
        await ContentIntelligenceService.extractEntitiesForAudit(
          params.auditId,
        );
      return { extracted };
    } catch (error) {
      console.warn(
        `Entity extraction failed for audit ${params.auditId}:`,
        error,
      );
      return { extracted: 0 };
    }
