...
export async function maybeSendSelfHostHeartbeat(
  ...
  try {
    ...
    await dependencies.sendHeartbeat(state.installId, {
      // The app is hosted-only now; this telemetry field is kept for backward
      // compatibility with the dashboard. It's always "cloudflare" since there
      // is no longer a local_noauth / docker path.
