 ...
 * is configured.
 *
 * When `OPENROUTER_BASE_URL` is set, requests go to that OpenAI/OpenRouter-
 * compatible gateway instead of openrouter.ai. Provider-routing options
 * (`order`/`zdr`/`allow_fallbacks`) are OpenRouter-specific and are dropped
 * in that mode so the custom gateway does not reject the request body.
 */
