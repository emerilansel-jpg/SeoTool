...
// Domain intersection returns the same keyword_data block as ranked keywords,
// plus a SERP element per target. We request intersections:false (keywords the
// competitor ranks for that the project domain does not), so only
// first_domain_serp_element (the competitor/target_1 ranking) is populated.
...
/**
 * Keywords target1 ranks for that target2 does not (intersections:false).
 * Used to compute a content/keyword gap against a competitor domain: target1
 * is the competitor, target2 is the project's own domain.
 */
