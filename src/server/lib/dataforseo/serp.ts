...
export interface RankCheckResult {
  ...
  url: string | null;
  serpFeatures: string[];
  /** Top-N organic SERP items for persistence (competitor snapshot). */
  serpItems?: SerpSnapshotRow[];
}

/** A single organic SERP result, reduced to the fields we persist. */
...
/** Maximum organic items to persist per rank check (controls DB write size). */
const MAX_SERP_ITEMS_TO_PERSIST = 20;

/** Extract top-N organic items from a full SERP response for persistence.
 *  Pure function — unit-testable, no I/O. */
export function extractSerpItems(
  items: SerpLiveItem[],
  targetDomain: string,
  maxItems = MAX_SERP_ITEMS_TO_PERSIST,
): SerpSnapshotRow[] {
  ...
  return items
    ...
    .map((item) => {
      ...
      const isTracked =
        domain != null && (domain === target || domain.endsWith(`.${target}`));
      return {
