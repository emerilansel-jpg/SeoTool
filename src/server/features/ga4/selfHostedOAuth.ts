...
async function verifyState(state: string, clientSecret: string) {
  ...
  if (parsed.exp < Date.now()) {
    throw new AppError("VALIDATION_ERROR", "Expired Google Analytics state");
  }
...
function getGoogleAccountId(tokens: GoogleTokenResponse) {
  if (!tokens.id_token) {
    throw new AppError(
      "VALIDATION_ERROR",
      "Google did not return an ID token for Google Analytics.",
    );
  }
...
async function exchangeCode(input: {
  ...
  if (!response.ok) {
    throw new AppError(
      "VALIDATION_ERROR",
      "Google rejected the Google Analytics authorization code.",
    );
  }
...
async function _handleSelfHostedGa4OAuthCallback(input: {
  request: Request;
  user: SelfHostedGa4User;
  publicOrigin: string;
}) {
  ...
  if (!stateParam) {
    return new Response("Missing Google Analytics OAuth state", {
      status: 400,
    });
  }
  ...
  if (state.userId !== input.user.userId) {
    return new Response("Google Analytics OAuth user mismatch", {
      status: 403,
    });
  }
  ...
  if (!code) {
    return new Response("Missing Google Analytics OAuth code", {
      status: 400,
    });
  }
