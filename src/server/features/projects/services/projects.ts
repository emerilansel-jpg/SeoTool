...
export async function listProjectsEnsuringOne(organizationId: string) {
  // E2E bypass: return a mock project to avoid DB queries for the fake org.
...
export async function createProject(
  ...
  // Enforce the plan's project-count limit before creating.
  await assertGaugeFeature(organizationId, "projects");
...
export async function getProjectForOrganization(
  ...
  // E2E bypass: return mock project data to avoid DB queries for the fake org.
  if (organizationId === "e2e-org-id") {
    return {
      id: projectId,
      organizationId: "e2e-org-id",
