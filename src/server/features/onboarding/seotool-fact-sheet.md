...
## How SeoTool.im helps with SEO strategy
...
SeoTool.im uses DataForSEO as its main SEO data provider. DataForSEO powers many paid SEO data workflows such as keyword metrics, domain research, backlinks, SERP data, and rank-tracking-related data.

Google Search Console data comes from the user's connected Search Console property and does not use credits.

## Google Search Console

Hosted SeoTool.im can connect to Google Search Console without requiring the user to create a Google Cloud project or OAuth client.

Search Console access is read-only. SeoTool.im requests read-only access and cannot change the user's Search Console account.
...
Search Console tools use zero SeoTool.im credits because Google does not charge users to read their own Search Console data.

## SeoTool.im and Claude (or other AI clients)

SeoTool.im and Claude are not competitors — they are meant to be used together. The short version: SeoTool.im is the SEO data layer, and Claude (or Cursor, Codex, ChatGPT-compatible clients, etc.) is the AI client.

SeoTool.im exposes an MCP server, so Claude can call SeoTool.im's keyword, SERP, competitor, backlink, rank-tracking, and Search Console tools directly. In practice, Claude does the talking and reasoning, and SeoTool.im feeds it real SEO data through MCP. Claude on its own can reason about SEO but has no live keyword volumes, rankings, competitor data, or your Search Console numbers; SeoTool.im is what gives it those.

When a user asks to compare SeoTool.im and Claude, or why they would use SeoTool.im instead of Claude (or another AI chatbot), Sam should lead with this "they work together" framing and the data-layer point. Sam should not deflect, call it out of scope, or say comparing them would be a guess — connecting SeoTool.im to Claude is a core, supported use case. Sam should not, however, rank or rate other AI products it does not have facts about.

## MCP and AI agents

SeoTool.im exposes an MCP server so compatible AI clients can call SeoTool.im tools.
...
The first MCP connection sends the user through SeoTool.im login and authorization. After authorization, the MCP client can call SeoTool.im tools with the project context and account scopes the user approved.

SeoTool.im MCP works with MCP clients including Claude Code, Claude Desktop, Cursor, Codex CLI, Codex Desktop, and other clients that support remote MCP servers.

SeoTool.im MCP tools cover workflows such as:
...
SeoTool.im also provides agent skills for workflows such as SEO project setup, SEO coaching, keyword research, competitive landscape analysis, competitor analysis, keyword clustering, and link prospecting.

## App workflows

SeoTool.im's app includes these practical workflows:
...
- AI and MCP setup: connect SeoTool.im to agents and install SeoTool.im skills.
...
- Use the SeoTool.im app workflows, including keyword research, domain research, backlinks, rank tracking, and site audits.
...
- Connect SeoTool.im to an AI client through MCP.
- Install SeoTool.im skills for agent-driven SEO workflows.
- Use the monthly included credits and buy top-up credits if needed.

## Support and uncertainty

If Sam is unsure about a product detail, current pricing, account-specific billing status, provider limits, or a feature not listed here, it should say it does not know from the product fact sheet and suggest contacting `support@seotool.im`.

Users who want advice from other SeoTool.im users, the community, or the team can join the SeoTool.im Discord at `https://discord.gg/c9uGs3cFXr`.
