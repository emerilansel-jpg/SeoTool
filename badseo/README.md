...
[SeoTool.im](https://seotool.im) site audit. Every page lists the audit issues it
should trigger, and a harness runs the real audit engine against a running copy
to check that it does.

Maintained by the team behind [SeoTool.im](https://seotool.im), an open-source SEO
tool.
...
Every issue type in the SeoTool.im audit engine is exercised by at least one page
...
The harness drives the **real** SeoTool.im crawl + issue-detection functions
