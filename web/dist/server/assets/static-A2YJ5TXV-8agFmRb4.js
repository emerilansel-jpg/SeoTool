import { r as removeUndefined } from "./chunk-ZMWYLUDP-DsjLwjNJ.js";
import { c as createContentHighlighter } from "./chunk-OTD7MV33-8NxzDOm0.js";
const STEMMERS = {
  arabic: "ar",
  armenian: "am",
  bulgarian: "bg",
...
async function searchSimple(db, query, params = {}) {
  ...
  const result = await search$1(db, {
    term: query,
    tolerance: 1,
    ...params,
    boost: {
...
async function loadDB({
  ...
  async function init() {
    ...
    if (data.type === "i18n") {
      await Promise.all(
        Object.entries(data.data).map(async ([k, v2]) => {
