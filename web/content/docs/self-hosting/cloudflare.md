...
Host SeoTool.im on Cloudflare for internet-facing self-hosting across multiple devices or with your team. One deploy command provisions everything, including the Cloudflare Access login gate. Works on Cloudflare's free plan.
...
## 1) Clone your SeoTool.im repo

Fork `emerilansel-jpg/SeoTool` on GitHub if you want a repo you control, then clone it locally:
...
3. SeoTool.im should load after login.

If login fails, re-check `ACCESS_ALLOWED_EMAILS` and redeploy.

## Connect the MCP server through Cloudflare Access

Use the same Cloudflare Access application that protects your SeoTool.im Worker. Managed OAuth is required for MCP clients and is not enabled by default.

1. Open Cloudflare Zero Trust.
2. Go to `Access controls` -> `Applications`.
3. Find your SeoTool.im application, then select `Edit`.
...
## Give teammates access to SeoTool.im

Add the teammate to `ACCESS_ALLOWED_EMAILS` in `.env.selfhost` and redeploy. Everyone allowed through shares one SeoTool.im workspace.

## Updating to the latest SeoTool.im version
...
- [Operations](https://github.com/emerilansel-jpg/SeoTool/blob/main/docs/SELF_HOSTING_CLOUDFLARE_OPERATIONS.md): telemetry and other day-to-day tasks.
- [Legacy deployments](https://github.com/emerilansel-jpg/SeoTool/blob/main/docs/SELF_HOSTING_CLOUDFLARE_LEGACY.md): maintenance for installs created with the retired Deploy-button or manual Wrangler flows.
