...
The Search Console tools use zero credits — Google doesn't charge you to read your own data, so reading it through SeoTool.im never burns usage. They're included with the SeoTool.im All Access membership, alongside the credit-based tools (keyword research, rank tracking, backlinks) that run through the same MCP endpoint. If you'd rather not pay at all, SeoTool.im is open source and free to self-host — see the FAQ below.
