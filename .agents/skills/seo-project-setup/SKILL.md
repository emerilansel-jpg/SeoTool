...
### 5. Verify SeoTool.im MCP

After the user has described the company, website, goals, and positioning, check that SeoTool.im MCP is configured and mapped to the right project:
...
5. If the MCP is unavailable, tell the user to connect SeoTool.im MCP before continuing with live SeoTool.im data.
...
After intake, recommend one next SeoTool.im workflow:
